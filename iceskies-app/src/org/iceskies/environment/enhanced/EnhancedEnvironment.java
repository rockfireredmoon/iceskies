...
public class EnhancedEnvironment extends AbstractEnvironmentImpl implements PropertyChangeListener {
	...
	public void setEnvironment(AbstractEnvironmentConfiguration environmentConfiguration) {
		...
		this.environmentConfiguration = (EnhancedEnvironmentConfiguration) environmentConfiguration;
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (evt.getPropertyName().equals(EnhancedEnvironmentConfiguration.PROP_CLOUDS)) {
			...
			LOG.info(String.format("New celestial body alpha is %4.2f", environmentConfiguration.getCelestialBodyAlpha()));
			skyDome.setCelestialBodyAlpha((Float) environmentConfiguration.getCelestialBodyAlpha());
			skyDome.completeTransition();
		} else if (evt.getPropertyName().equalsIgnoreCase(EnhancedEnvironmentConfiguration.PROP_CELESTIAL_BODY_DIRECTION)) {
			LOG.info(String.format("Celestial Body direction now %3.3f", environmentConfiguration.getCelestialBodyDirection()));
	...
	protected void keepSunAtDistanceFromCamera(float tpf) {
		if (sun != null && (!environmentConfiguration.isControlDirectionalPosition() || !environmentConfiguration.isSkyDome())) {
			...
			if (sun == null) {
				gameNode.attachChild(sun = new Sun(environmentLight, assetManager) {
					...
					protected void onSunApply() {
						...
						((EnhancedEnvironmentConfiguration) env.getEnvironment()).setDirectionalPosition(getLocalTranslation());
					}
				});
			}
			sun.setSunColor(environmentLight.getSunColor());
			sun.setLocalTranslation(environmentLight.getSunRepresentationPosition());
			LOG.info(String.format("Added sun representation at %s (dir is %s) with color of %s", sun.getLocalTranslation(),
					environmentLight.getSunDirection(), environmentLight.getSunColor()));
	...
	private void configureSkydome() {
		...
		if (skyDome != null && (lastSkyMap == null || !lastSkyMap.equals(environmentConfiguration.getSkyMap())
				...
				|| lastCelestialBodyMap == null || !lastCelestialBodyMap.equals(environmentConfiguration.getCelestialBodyMap()))) {
			removeSky();
		}
		...
		if (skyDome == null) {
			...
			skyDome = new SkyDomeControl(assetManager, camera, "Environment/SkyDome.j3o",
					environmentConfiguration.absolutize(lastSkyMap), environmentConfiguration.absolutize(lastCelestialBodyMap),
					environmentConfiguration.absolutize(lastCloudsMap), environmentConfiguration.absolutize(lastFogAlphaMap)) {
				@Override
		...
		LOG.info(String.format("     Celestial Body Direction: %4.3f", environmentConfiguration.getCelestialBodyDirection()));
		LOG.info(String.format("     Celestial Body Scale: %4.3f", environmentConfiguration.getCelestialBodyScale()));
		LOG.info(String.format("     Day: %s", UIUtil.toHexString(environmentConfiguration.getSkyColor())));
		...
		LOG.info(String.format("         Directional: %s", UIUtil.toHexString(environmentConfiguration.getDirectionalColor())));
		LOG.info(String.format("         Ambient: %s", UIUtil.toHexString(environmentConfiguration.getAmbientColor())));
		...
		LOG.info(String.format("         Color: %s", UIUtil.toHexString(environmentConfiguration.getFogColor())));
