...
public class Env extends AbstractCommand {
	...
	public void init(ConsoleAppState console) {
		...
		options.addOption("v", "verbose", false, "Be verbose where possible.");
			...
			if (commandLine.hasOption('e')) {
				...
				for (String e : manager.getEnvironmentConfigurations()) {
					if (commandLine.hasOption('v')) {
						AbstractEnvironmentConfiguration env = manager.getEnvironmentConfiguration(e);
						bui.append(env.toString() + "\n");
					}
					else
						console.output(e + "\n");
				}
				console.output(bui.toString());
				return true;
			} else {
				if (args.length > 0) {
