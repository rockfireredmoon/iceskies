...
import org.iceskies.environment.SkiesConstants;
...
public class LegacyEnvironment extends AbstractEnvironmentImpl<LegacyEnvironmentConfiguration>
	...
	@Override
	public void setEnvironment(LegacyEnvironmentConfiguration environmentConfiguration) {
		checkNotCleanedUp();
	...
	private void updateAll() {
		...
		/*
		 * Some interpolators should not just be removed, as they may need to
		 * person some final operation such as removing a skynode.
		 */
		...
		if (environmentLight != null) {
			...
			if (!Objects.equal(currentAmbient, ambient)) {
				LOG.info(String.format("Ambient changed to %s, interpolating current lighting towards this to value",
						ambient));
			...
			ColorRGBA currentDirectional = environmentLight.isDirectionalAllowed()
					? environmentLight.getSunColor().clone() : ColorRGBA.Black;
			if (!Objects.equal(currentDirectional, directional)) {
				LOG.info(
						String.format("Directional changed to %s, interpolating current lighting towards this to value",
								directional));
		...
		if (pp == null) {
			...
			pp.setFogFilterMode(FogFilterMode.OGL);

			final FogFilter fogFilter = (FogFilter) pp.getFogFilter();
			if (fogFilter == null) {
				LOG.severe("No fog filter found, fog effects will not be visible.");
			} else {
				if (this.fog != null && (fog == null || !fog.isEnabled())) {
					...
					// interpolators.add(new
					// FogColourInterpolator(this.fog.getColor(),
					// ColorRGBA.Black,
					// fogFilter));
					...
					LOG.info(String.format(
							"Fog enabled (%s, Color: %s to %s, Exclude Sky: %s, Start: %f, End: %f, Exp: %f",
							fogFilter.getFogMode(), fogFilter.getFogColor(), fog.getColor(), fog.isExcludeSky(),
							fog.getStart(), fog.getEnd(), fog.getExp()));
					...
					if (!Objects.equal(fogFilter.getFogColor(), fog.getColor())) {
						interpolators.add(
								new FogColourInterpolator(fogFilter.getFogColor(), fog.getColor().clone(), fogFilter));
					}
					fogFilter.setEnabled(true);
					...
					if (fogFilter.getFogStartDistance() != fogStart) {
						interpolators
								.add(new FogStartInterpolator(fogFilter.getFogStartDistance(), fogStart, fogFilter));
					}
					...
					LOG.info(String.format(
							"Fog updated (%s, Color: %s to %s, Exclude Sky: %s, Start: %f, End: %f, Exp: %f",
							fogFilter.getFogMode(), fogFilter.getFogColor(), fog.getColor(), fog.isExcludeSky(),
							fog.getStart(), fog.getEnd(), fog.getExp()));
				}
		...
		if (!interpolators.isEmpty()) {
			LOG.info(String.format("Environment has changed (%d interpolators to run), triggering reblend",
					interpolators.size()));
			blendProgress = 0;
		}
	...
	protected float getInterval(AudioQueue queue, PlaylistType type, String resource) {
		if (type == PlaylistType.AMBIENT_MUSIC) {
			return environmentConfiguration.getAmbientMusicDelay();
		}
		return 0;
	}
	...
	protected float getGain(AudioQueue queue, PlaylistType type, String resource) {
		AdjustChannel ac = environmentConfiguration.getAdjustChannelForPath(resource, assetManager);
		return ac == null ? 1 : ac.getAmount();
	}
