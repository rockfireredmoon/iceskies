...
public class EnvironmentSwitcherAppState extends IcemoonAppState<IcemoonAppState<IcemoonAppState<?>>> {
	...
	public EnvironmentSwitcherAppState(Preferences appPrefs, String defaultEnvironment, EnvironmentLight el,
			Node gameNode, Node weatherNode) {
		super(appPrefs);
		this.el = el;
		...
		if (environments.size() > 0) {
			...
			while (iterator.hasNext()) {
				...
				if (EnvironmentManager.get(assetManager).isEnvironment(env)) {
					...
					EnvironmentGroupConfiguration envConfigs = EnvironmentManager.get(assetManager)
							.getEnvironments(env);
				...
				try {
					AbstractEnvironmentConfiguration e = EnvironmentManager.get(assetManager)
							.getEnvironmentConfiguration(env);
					configs.add(e);
				} catch (IllegalArgumentException iae) {
					LOG.log(Level.SEVERE, "Failed to load environment.", iae);
				}
			}
			...
			if (force || !Objects.equals(configs, this.configs)) {
				...
				LOG.info(String.format("Switching to highest priority environment %s [%s] (blend %f)", env,
						environmentConfiguration, blendTime));
				try {
	...
	public void update(float tpf) {
		...
		if (targetDirection != null) {
			...
			if (blendProgress >= blendTime) {
				...
				el.setSunDirection(origDirection.clone().interpolateLocal(targetDirection, amt));
			}
		}
	}

	protected EnvironmentAppState getState() {
		return app == null || app.getStateManager() == null ? null
				: app.getStateManager().getState(EnvironmentAppState.class);
	}
